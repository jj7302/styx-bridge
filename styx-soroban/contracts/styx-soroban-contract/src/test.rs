...
fn create_claimable_balance_contract<'a>(
    e: &Env,
    contract_id: &Address,
) -> ClaimableBalanceContractClient<'a> {
...
fn basic_initalize_test() {
    let env = Env::default();
    let client = ClaimableBalanceContractClient::new(
        &env,
        &env.register_contract(None, ClaimableBalanceContract),
    );
